         ...
         [:div {:class "grid min-w-0 gap-6 xl:grid-cols-2 2xl:grid-cols-3"}
          ...
          [:section {:class "card min-w-0 border border-base-300 bg-base-100 shadow-md xl:col-span-2 2xl:col-span-1"}
           [:div {:class "card-body min-w-0 gap-4"}
            ...
            [:figure {:class "diff w-full aspect-[16/9] rounded-lg border border-base-300 bg-base-200/70"}
             [:div {:class "diff-item-1 flex items-center justify-center p-4 text-sm text-base-content/80"}
              [:div {:class "max-w-full space-y-2 text-left"}
               [:div {:class "font-semibold"} "Greek tradition"]
               [:p "Final cause names purpose as teleological endpoint."]
               [:p "Ethics focuses on cultivated habit and practical reason."]]]
             [:div {:class "diff-item-2 flex items-center justify-center p-4 text-sm text-base-content/80"}
              [:div {:class "max-w-full space-y-2 text-left"}
               [:div {:class "font-semibold"} "Modern framing"]
               [:p "Translator emphasizes scientific method and causation language."]
               [:p "Commentary stresses context over universal teleology."]]]
